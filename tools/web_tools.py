...
def _get_backend() -> str:
    ...
    if configured in {"parallel", "firecrawl", "tavily", "exa", "searxng", "crawl4ai", "brave-free", "ddgs"}:
        return configured
...
async def web_extract_tool(
    ...
    try:
        ...
        if not safe_urls:
            ...
            # async (parallel, firecrawl), others sync (exa, tavily, crawl4ai).
            from agent.web_search_registry import (
                    ...
                    return json.dumps(
                        {
                            ...
                            "error": (
                                ...
                                "tavily, exa, parallel, or crawl4ai."
                            ),
                        },
                        ensure_ascii=False,
                    )
...
def check_web_api_key() -> bool:
    ...
    if configured in {"exa", "parallel", "firecrawl", "tavily", "searxng", "crawl4ai", "brave-free", "ddgs"}:
        return _is_backend_available(configured)
    return any(
        _is_backend_available(backend)
        for backend in ("exa", "parallel", "firecrawl", "tavily", "searxng", "crawl4ai", "brave-free", "ddgs")
    )

...
class SessionDB:
        ...
        def _do(conn):
            conn.execute(
                """INSERT OR IGNORE INTO sessions (id, source, user_id, model, model_config,
                   system_prompt, parent_session_id, profile, cwd, started_at)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (
                    session_id,
                    source,
        ...
        if project_compression_tips and not include_children:
            ...
            for s in sessions:
                ...
                for key in (
                    ...
                    "model", "model_config", "system_prompt", "cwd",
                ):
                    if key in tip_row:
    ...
    def get_messages(
        ...
        audit / debug views of rewound history).

        Ordered by AUTOINCREMENT id (true insertion order) rather than
        timestamp to avoid WSL2 clock-regression ordering bugs. The
        API-server pagination layer may pass ``limit``, ``offset`` and
        ``order``.
        """
        active_clause = "" if include_inactive else " AND active = 1"
        order_clause = "ORDER BY id DESC" if order == "desc" else "ORDER BY id"

        def _fetch():
            query = f"SELECT * FROM messages WHERE session_id = ?{active_clause} {order_clause}"
            params = [session_id]
